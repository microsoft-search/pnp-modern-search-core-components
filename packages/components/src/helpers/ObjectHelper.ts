...
export class ObjectHelper {
    /**
     * Get object property value by its deep path using jsPath predicate.
     * @param object the object containg the property path
     * @param path the property path to get
     * @param delimiter if multiple matches are found, specifiy the delimiter character to use to separate values in the returned string 
    ...
    public static setPropertyByPath (object: any, path: string, value: unknown) {

        const [head, ...rest] = path.split(".");
        return {
            ...object,
            [head]: rest.length
                ? ObjectHelper.setPropertyByPath(object[head], rest.join("."), value)
                : value
        };
    }

    /**
     * Returns the property with original type according to the provided object path 
     * @param object the source object
     * @param path the property path
     * @returns 
     */
    public static getPropertyByPath(object: any, path: string): any {
